  ...
  return (
    <div style={s.wrapper}>
      ...
      <main style={s.main}>
        ...
        <div style={s.calGrid}>
          {calDays.map((day,i)=>{
            const dt=turnosDelDia(day),isS=isSameDay(day,selectedDay),td=isToday(day),isCM=isSameMonth(day,currentMonth)
            return (
          ...
          <div key={t.id} style={{...s.tCard,...(selectedTurno?.id===t.id?s.tCardA:{})}} onClick={()=>setSelectedTurno(selectedTurno?.id===t.id?null:t)}>
            ...
            {selectedTurno?.id===t.id&&(
              <div style={s.tDetail} className="animate-in">
                {t.descripcion&&<div style={s.dRow}><span>📦 Mercadería</span><strong>{t.descripcion}</strong></div>}
...
const s: Record<string, CSSProperties> = {
  ...
  logoRow:{display:'flex',alignItems:'center',gap:'8px',marginBottom:'20px'},logoText:{fontWeight:'700',fontSize:'16px',flex:1},
  ...
  statNum:{fontSize:'22px',fontWeight:'700',lineHeight:'1'},statLabel:{fontSize:'11px',color:'var(--text2)',marginTop:'4px'},
  ...
  calHead:{display:'flex',alignItems:'center',gap:'12px',marginBottom:'16px'},calTitle:{fontSize:'20px',fontWeight:'700',textTransform:'capitalize',flex:1},
  ...
  dHeads:{display:'grid',gridTemplateColumns:'repeat(7, 1fr)',marginBottom:'4px'},dHead:{textAlign:'center',fontSize:'12px',fontWeight:'600',color:'var(--text3)',padding:'4px 0',textTransform:'uppercase'},
  ...
  cCellO:{opacity:0.35},cCellS:{borderColor:'var(--accent)',background:'var(--accent-light)'},cCellT:{borderColor:'var(--accent)'},
  ...
  tCardA:{borderColor:'var(--accent)'},badge:{fontSize:'11px',padding:'2px 8px',borderRadius:'20px',fontWeight:'600'},
